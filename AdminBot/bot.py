...
def yookassa_set_shop_id(message: Message):
    ...
    from Utils.yookassa import save_yookassa_settings
    settings = utils.all_configs_settings()
    secret_key = settings.get('yookassa_secret_key', '')

    if save_yookassa_settings(USERS_DB, shop_id, secret_key):
        bot.send_message(message.chat.id, MESSAGES['YOOKASSA_SETTINGS_SAVED'], reply_markup=markups.main_menu_keyboard_markup())
    else:
        bot.send_message(message.chat.id, MESSAGES['ERROR_UNKNOWN'], reply_markup=markups.main_menu_keyboard_markup())

def yookassa_set_secret_key(message: Message):
    if message.text == KEY_MARKUP['CANCEL']:
        bot.send_message(message.chat.id, MESSAGES['CANCELED'], reply_markup=markups.main_menu_keyboard_markup())
        return
...
def _handle_admin_callback(call: CallbackQuery):
    ...
    # YooKassa Settings
        ...
        bot.edit_message_text(MESSAGES['YOOKASSA_SETTINGS'], call.message.chat.id, call.message.message_id,
                              reply_markup=markups.yookassa_settings_markup())

    elif key == "yookassa_set_shop_id":
        bot.send_message(call.message.chat.id, MESSAGES['YOOKASSA_SHOP_ID'], reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(call.message, yookassa_set_shop_id)
